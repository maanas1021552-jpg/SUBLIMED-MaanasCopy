...
Experience has shown that a reasonably good model can be produced with par->pIntensity ~ a few thousand grid points (or ~10,000 for a more precise model). Radiation trapping effects tend to be very small, so useEP = 0 can be set for most modlecules (apart from H2O), which allows the code to run faster. The default LIME 1.9.3 raytacer "snaps" to the nearest grid point during raytracing, which tends to slightly overestimate the flux in a 1/rho cometary coma. This issue can be mitigated by increasing the grid density (to say, 50,000 grid points), and/or by using par->traceRayAlgorithm = 1, which interpolates the grid points instead (but is much slower). 
